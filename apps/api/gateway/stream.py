...
def _detect_report_request(user_query: str) -> tuple[str | None, dict]:
    ...
    report_patterns = {
        ...
        # Generic patterns - require report-related keywords to avoid shadowing agents
        "citadel_technical_generic": [
            r"technical.*analysis.*report",
            r"technical.*report.*card",
            r"citadel.*style.*technical",
        ],
        "goldman_screener_generic": [
            r"stock.*screen.*report",
            r"goldman.*style.*screen",
        ],
        "morgan_dcf_generic": [
            r"dcf.*valuation.*report",
            r"discounted.*cash.*flow.*memo",
            r"full.*dcf.*analysis",
        ],
        "jpm_earnings_generic": [
            r"earnings?.*analysis.*report",
            r"earnings?.*brief",
            r"jpm.*style.*earnings",
        ],
        "blackrock_builder_generic": [
            r"portfolio.*builder.*report",
            r"asset.*allocation.*memo",
            r"blackrock.*style.*allocation",
        ],
        "harvard_dividend_generic": [
            r"dividend.*strategy.*report",
            r"harvard.*style.*dividend",
        ],
        "bain_competitive_generic": [
            r"competitive.*analysis.*report",
            r"industry.*deep.*dive",
            r"bain.*style.*competitive",
        ],
        "renaissance_pattern_generic": [
            r"pattern.*finder.*report",
            r"renaissance.*style.*pattern",
        ],
        "mckinsey_macro_generic": [
            r"macro.*economic.*report",
            r"economic.*impact.*memo",
            r"mckinsey.*style.*macro",
        ],
        "bridgewater_risk_generic": [
            r"risk.*assessment.*report",
            r"portfolio.*risk.*analysis",
            r"bridgewater.*style.*risk",
        ],
    }
